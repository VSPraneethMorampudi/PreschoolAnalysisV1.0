...
export const testGeoServerWFS = async (layerId: string, typeName: string) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `HTTP error for ${layerId}: ${response.status} ${response.statusText}`
      );
      return false;
    }
...
export const initializeOpenLayersMap = (
  target: HTMLElement,
  ...
  const map = new Map({
    ...
    view: new View({
      ...
      zoom: 6.8,
      minZoom: 4,
      maxZoom: 23, // Increased maximum zoom level
...
export const createLayerFromConfig = (
  config: LayerConfig
): TileLayer<TileWMS> | VectorLayer<VectorSource> => {
  switch (config.type) {
    case "wms":
    ...
    case "wfs":
      ...
      fetch(config.url, {
        ...
        .then((response) => {
          if (!response.ok) {
            console.error(
              `HTTP error for ${config.id}: ${response.status} ${response.statusText}`
            );
        ...
        .then((data) => {
          if (data && data.features) {
            console.log(
              `WFS layer ${config.id} has ${data.features.length} features`
            );
          }
        })
        .catch((error) => {
...
export const switchBasemap = (map: Map, basemapId: string): void => {
  ...
  const baseLayerIndex = layers
    .getArray()
    .findIndex((layer) => layer.get("id") === "base");
...
export const clusterPoints = (
  ...
  points.forEach((point) => {
    ...
    if (cluster.count > 1) {
      const avgLon =
        cluster.points.reduce((sum, p) => sum + p.coordinates[0], 0) /
        cluster.count;
      const avgLat =
        cluster.points.reduce((sum, p) => sum + p.coordinates[1], 0) /
        cluster.count;
