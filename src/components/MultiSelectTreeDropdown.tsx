...
const IconPlaceholder: React.FC<{ type: string; className?: string }> = ({
  type,
  className = "w-4 h-4",
}) => {
...
export const MultiSelectTreeDropdown: React.FC<
  MultiSelectTreeDropdownProps
> = ({ data, onSelectionChange, searchTerm = "" }) => {
  const {
    selectedLayers,
    expandedNodes,
    selectedLanguage,
  ...
  const isGroupPartiallySelected = (node: LayerTreeNode): boolean => {
    const childLayerIds = getAllChildLayerIds(node);
    const selectedChildIds = childLayerIds.filter((id) =>
      selectedLayers.includes(id)
    );
    return (
      selectedChildIds.length > 0 &&
      selectedChildIds.length < childLayerIds.length
    );
  };
  ...
  const isGroupFullySelected = (node: LayerTreeNode): boolean => {
    ...
    return (
      ...
      childLayerIds.every((id) => selectedLayers.includes(id))
    );
  };
  ...
  const renderTreeNode = (
    ...
    const hasMatchingChildren =
      ...
      node.children!.some(
        (child) =>
          matchesSearch(child) ||
          (child.children && child.children.some(matchesSearch))
      );
    ...
    const isIndeterminate =
      node.type === "group" && isGroupPartiallySelected(node);
          ...
          <div className="flex items-center space-x-2 flex-1 min-w-0">
            <Checkbox
              ...
              className={`
                ...
                ${
                  isIndeterminate
                    ? "data-[state=indeterminate]:bg-blue-500"
                    : ""
                }
              `}
            ...
            <div className="flex items-center space-x-1.5 flex-1 min-w-0">
              <div className="flex-1 min-w-0 ml-1">
                <span
                  className={`text-[11px] font-medium text-gray-700 truncate block ${
