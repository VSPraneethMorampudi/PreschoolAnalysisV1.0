...
const DISTANCE_LABEL_MIN_ZOOM = 12; // Minimum zoom level to show distance labels
  ...
  (
    ...
    const createUncoveredMarkersLayer = (uncoveredAnganwadis: any[]) => {
      ...
      console.log(
        `✅ Created ${
          features.length
        } uncovered markers as black circles with z-index ${layer.getZIndex()}`
      );
    };
    ...
    const performGapAnalysis = () => {
      ...
      if (!aSrc || !sSrc) {
        alert(
          "❌ Data sources not ready. Please wait for layers to load completely."
        );
        return;
      }
      ...
      console.log(
        `✅ Data ready: ${anganwadiFeatures.length} anganwadis, ${schoolFeatures.length} schools`
      );
      ...
      try {
        ...
        const railFeatures = railLayer
          ? (railLayer.getSource() as VectorSource)?.getFeatures() || []
          : [];
        const roadFeatures = roadLayer
          ? (roadLayer.getSource() as VectorSource)?.getFeatures() || []
          : [];
        const riverFeatures = riverLayer
          ? (riverLayer.getSource() as VectorSource)?.getFeatures() || []
          : [];

        console.log(
          `🔍 Infrastructure layers loaded: Rail(${railFeatures.length}), Roads(${roadFeatures.length}), Rivers(${riverFeatures.length})`
        );

        console.log(
          "🔍 Starting Gap Analysis - Identifying Uncovered Anganwadis..."
        );
        ...
        const gapLayer = mapRef.current
          ?.getAllLayers()
          .find((l) => l.get("name") === "gapAnalysisLayer") as VectorLayer;
        if (gapLayer) {
        ...
        anganwadis.forEach((aw, index) => {
          if (index < 5)
            console.log(
              `📍 Processing anganwadi ${index + 1}:`,
              aw.getProperties()?.awc_name || "Unknown"
            );
          try {
            totalAnalyzed++;
            ...
            if (!awGeo || !awProps) {
              console.warn(
                `Skipping anganwadi ${totalAnalyzed} - invalid geometry or properties`
              );
              return;
            }
            ...
            schools.forEach((school) => {
              try {
                ...
                const distance = turf.distance(awGeo, schoolGeo, {
                  units: "kilometers",
                });
            ...
            if (!hasSchoolAccess || isObstacleBlocked) {
              const gapReason = !hasSchoolAccess
                ? "No school within 5km"
                : isObstacleBlocked
                ? `Blocked by: ${blockingObstacles.join(", ")}`
                : "Unknown";
              ...
              uncoveredAnganwadis.push({
                ...
                recommendation: isObstacleBlocked
                  ? "Infrastructure: Remove barriers or provide alternative access"
                  : nearestDistance > 10
                  ? "Urgent: New school needed"
                  : nearestDistance > 7.5
                  ? "High Priority: Transport or new school"
            ...
            console.error(
              `Error processing anganwadi ${totalAnalyzed}:`,
              awError
            );
          }
        });
        ...
        if (gapAnalysisLayer) {
          ...
          uncoveredAnganwadis.forEach((gap) => {
            ...
            const buffer = turf.buffer(awGeo, COVERAGE_RADIUS, {
              units: "kilometers",
            });
            const bufferFeature = readFeatures3857({
        ...
        const gapReport = {
          ...
          coveragePercentage: (
            ((totalAnalyzed - uncoveredAnganwadis.length) / totalAnalyzed) *
            100
          ).toFixed(1),
          gapPercentage: (
            (uncoveredAnganwadis.length / totalAnalyzed) *
            100
          ).toFixed(1),
          criticalGaps: uncoveredAnganwadis.filter(
            (g) => g.gapSeverity === "Critical"
          ).length,
          highPriorityGaps: uncoveredAnganwadis.filter(
            (g) => g.gapSeverity === "High"
          ).length,
          moderateGaps: uncoveredAnganwadis.filter(
            (g) => g.gapSeverity === "Moderate"
          ).length,
          infrastructureBlocked: uncoveredAnganwadis.filter(
            (g) => g.gapSeverity === "Infrastructure Blocked"
          ).length,
          roadBlocked: uncoveredAnganwadis.filter((g) =>
            g.blockingObstacles?.includes("Major Road")
          ).length,
          riverBlocked: uncoveredAnganwadis.filter((g) =>
            g.blockingObstacles?.includes("River")
          ).length,
        ...
        const uncoveredData = uncoveredAnganwadis.map((gap) => ({
          ...
          obstacleStatus:
            gap.gapSeverity === "Infrastructure Blocked"
    ...
    const checkInfrastructureLayersStatus = () => {
      ...
      if (railLayer) {
        ...
        console.log(
          `🚂 Rail Layer: ${
            railLayer.getVisible() ? "Visible" : "Hidden"
          } - ${railCount} features loaded`
        );
      } else {
      ...
      if (roadLayer) {
        ...
        console.log(
          `🛣️ Road/Highway Layer: ${
            roadLayer.getVisible() ? "Visible" : "Hidden"
          } - ${roadCount} features loaded`
        );
      } else {
      ...
      if (riverLayer) {
        ...
        console.log(
          `🌊 River Layer: ${
            riverLayer.getVisible() ? "Visible" : "Hidden"
          } - ${riverCount} features loaded`
        );
      } else {
      ...
      return {
        rail: {
          exists: !!railLayer,
          visible: railLayer?.getVisible(),
          features: railLayer
            ? (railLayer.getSource() as VectorSource)?.getFeatures().length
            : 0,
        },
        road: {
          exists: !!roadLayer,
          visible: roadLayer?.getVisible(),
          features: roadLayer
            ? (roadLayer.getSource() as VectorSource)?.getFeatures().length
            : 0,
        },
        river: {
          exists: !!riverLayer,
          visible: riverLayer?.getVisible(),
          features: riverLayer
            ? (riverLayer.getSource() as VectorSource)?.getFeatures().length
            : 0,
        },
      };
    };
    ...
    const exportGapAnalysisAlerts = (gapData: any) => {
      if (
        !gapData ||
        !gapData.uncoveredDetails ||
        gapData.uncoveredDetails.length === 0
      ) {
        alert(
          "No gap analysis alerts available for export. Please run gap analysis first."
        );
        return;
      }
      ...
      const alertRows = gapData.uncoveredDetails.map(
        (gap: any, index: number) => {
          const getSeverity = () => {
            ...
            if (gap.blockingObstacles?.length > 0)
          ...
          const getRecommendedAction = () => {
            ...
            switch (severity) {
              case "CRITICAL":
                return "Immediate new primary school establishment within 3-5km radius";
              case "HIGH":
                return "Priority area for educational infrastructure development";
              case "INFRASTRUCTURE_BLOCKED":
                return "Address infrastructure barriers and improve connectivity";
              default:
                return "Monitor for future development planning";
            }
          };
          ...
          const getTimeline = () => {
            ...
            switch (severity) {
              case "CRITICAL":
                return "6-12 months (Immediate)";
              case "HIGH":
                return "12-24 months (Priority)";
              case "INFRASTRUCTURE_BLOCKED":
                return "12-18 months (Infrastructure dependent)";
              default:
                return "24-36 months (Long-term planning)";
            }
          };
          ...
          return [
            ...
            csvEscape(
              gap.gapReason || "No primary school within acceptable range"
            ),
      ...
      if (link.download !== undefined) {
        ...
        console.log(
          `✅ Gap analysis alerts exported: ${gapData.uncoveredDetails.length} records`
        );
        alert(
          `✅ Gap Analysis Alerts Exported Successfully!\n\n📊 Report Details:\n• Total Alerts: ${
            gapData.uncoveredDetails.length
          }\n• Critical: ${gapData.criticalGaps}\n• High Priority: ${
            gapData.highPriorityGaps
          }\n• Infrastructure Blocked: ${
            gapData.infrastructureBlocked
          }\n\nFile saved as: gap_analysis_alerts_${
    ...
    const forceTriangleStyle = () => {
      ...
      if (anganwadiSource) {
        ...
        console.log(
          `🔺 Forcing ${anganwadis.length} anganwadis to triangle style`
        );
    ...
    useEffect(() => {
      if (!mapRef.current) {
        ...
        try {
          ...
          // Add zoom change handler for district labels and distance labels
          map.getView().on("change:resolution", () => {
            const districtLayer = mapRef.current
              ?.getAllLayers()
              .find(
                (l) => l.get("name") === "districtLayer"
              ) as VectorLayer<VectorSource>;
            if (districtLayer) {
              districtLayer.changed(); // This will trigger style recalculation
            }

            // Update distance labels visibility
            const connectionLayer = mapRef.current
              ?.getAllLayers()
              .find(
                (l) => l.get("name") === "connectionLayer"
              ) as VectorLayer<VectorSource>;
            if (connectionLayer) {
              const source = connectionLayer.getSource();
              const distanceLabels =
                source
                  ?.getFeatures()
                  .filter((f) => f.get("type") === "distance_label") || [];
              distanceLabels.forEach((label) => label.changed()); // Trigger style recalculation for each label
            }
          });
          if (closer) {
            closer.onclick = () => {
          ...
          map.on("singleclick", (evt) => {
            ...
            if (feature && content) {
              ...
              if (feature.get("awc_name")) {
                point = {
                  id: feature.get("awc_code") || "unknown",
                  coordinates: feature.getGeometry().getCoordinates(),
                  properties: {
                    name: feature.get("awc_name") || "Unknown Anganwadi",
                    type: "Anganwadi Center",
                    category: "Primary",
                    district:
                      feature.get("project") ||
                      ...
                      "Unknown District",
                    village:
                      feature.get("vill_nam_e") ||
                      feature.get("village_na") ||
                      "Unknown Village",
                    accessRoute: !feature.get("hasObstacleRoute"),
                    obstacleInfo: feature.get("hasObstacleRoute")
                      ? "Route has obstacles"
                      : null,
                    connectedToSchool: feature.get("connectedToSchool"),
                    serviceBuffer: "5 km",
                  },
                };
              }
              // Handle uncovered anganwadi marker clicks
                ...
                point = {
                  id: feature.get("name") || "unknown",
                  coordinates: feature.getGeometry().getCoordinates(),
                  properties: {
                    name: feature.get("name") || "Unknown Anganwadi",
                    type: "Uncovered Anganwadi",
                    status: "UNCOVERED",
                    nearestSchool: feature.get("nearestDistance"),
                    obstacleStatus: feature.get("obstacleStatus"),
                    gapReason: feature.get("reason"),
                    severity: "Critical",
                    recommendation:
                      feature.get("nearestDistance") === "No schools within 5km"
              ...
              // Handle school clicks
                ...
                point = {
                  id: feature.get("school_code") || "unknown",
                  coordinates: feature.getGeometry().getCoordinates(),
                  properties: {
                    name: feature.get("school_nam") || "Unknown School",
                    type: "Primary School",
                    category: feature.get("category_o") || "Unknown Category",
                    district: feature.get("district_n") || "Unknown District",
                    village: feature.get("vill_nam_e") || "Unknown Village",
                    obstacleRisk: feature.get("obstacleRisk"),
                    obstacleInfo: feature.get("obstacleInfo"),
                    accessRoute: !feature.get("obstacleRisk"),
                  },
                };
              }
              // Handle connection line clicks
                ...
                point = {
                  ...
                  properties: {
                    ...
                    type: "Anganwadi-School Connection",
                    from: feature.get("anganwadiName"),
                    to: feature.get("schoolName"),
                    distance: feature.get("distance")?.toFixed(1) + " km",
                    bufferStatus: "Within 5 km radius",
                    hasObstacles: feature.get("hasObstacles"),
                    obstacleInfo: feature.get("obstacleInfo"),
                    accessRoute: !feature.get("hasObstacles"),
                  },
                };
              }
      ...
      const layer = upsertLayer("districtLayer", () => {
        ...
        return new VectorLayer({
          ...
          style: (f) => {
            ...
            const fontSize = Math.min(16, Math.max(12, zoom * 1.5));
    ...
    useEffect(() => {
      const layer = upsertLayer("railLayer", () => {
    ...
    useEffect(() => {
      const layer = upsertLayer("riverLayer", () => {
        ...
        return new VectorLayer({
          ...
          style: new Style({
            stroke: new Stroke({
              ...
              width: 2, // Very thin lines
            }),
            fill: new Fill({ color: "#4682B4" }), // Very light fill
          }),
        });
      });
      if (layer) {
    ...
    useEffect(() => {
      const layer = upsertLayer("roadLayer", () => {
        ...
        return new VectorLayer({
          ...
          style: new Style({
            stroke: new Stroke({
              color: "#F5330A", // Peru color for roads
              width: 2, // Very thin lines
            }),
            fill: new Fill({ color: "#F5330A" }),
          }),
        });
      });
      if (layer) {
      ...
      try {
        ...
        const processBatch = () => {
          ...
          try {
            ...
            const batchBuffered = turf.buffer(batchGeoJson, bufferRadius, {
              units: "kilometers",
            });
        ...
        const simpleBuffers = feats.map((feat) => {
          ...
          const circle = turf.circle(geo.geometry.coordinates, bufferRadius, {
            units: "kilometers",
          });
    ...
    const connectAnganwadiToNearestSchool = () => {
      ...
      console.log(
        `� Fast connecting ${anganwadis.length} anganwadis to nearest schools...`
      );
      ...
      if (conn) {
        ...
        const checkObstaclesNearby = (
          anganwadiGeo: any,
          schoolGeo: any
        ): { hasObstacles: boolean; riskTypes: string[] } => {
          ...
          try {
            ...
            const routeBuffer = turf.buffer(routeLine, 0.1, {
              units: "kilometers",
            });
            ...
            if (riverLayer && riverLayer.getVisible() && !hasObstacles) {
              ...
              for (const riverFeature of riverFeatures) {
                try {
                  ...
                  if (
                    riverGeo &&
                    turf.booleanIntersects(routeBuffer, riverGeo)
                  ) {
                    hasObstacles = true;
        ...
        anganwadis.forEach((anganwadi, index) => {
          try {
            ...
            const anganwadiName =
              anganwadi.get("awc_name") || `Anganwadi ${index + 1}`;

            console.log(`Processing ${anganwadiName}...`);

            // Create 5km buffer around anganwadi
            const anganwadiBuffer = turf.buffer(
              anganwadiGeo,
              BUFFER_RADIUS_KM,
              { units: "kilometers" }
            );
            ...
            schools.forEach((school) => {
              try {
                ...
                const distance = turf.distance(anganwadiGeo, schoolGeo, {
                  units: "kilometers",
                });
                ...
                if (distance <= BUFFER_RADIUS_KM) {
                  ...
                  const { hasObstacles, riskTypes } = checkObstaclesNearby(
                    anganwadiGeo,
                    schoolGeo
                  );
            ...
            console.log(
              `Found ${schoolsInBuffer.length} schools in buffer for ${anganwadiName}`
            );
            ...
            if (schoolsInBuffer.length > 0) {
              ...
              try {
                ...
                lineFeature.set(
                  "hasObstacles",
                  nearestSchool.hasObstacleIntersection
                );
                lineFeature.set(
                  "obstacleInfo",
                  nearestSchool.obstacleTypes.join(", ")
                );
                ...
                if (nearestSchool.hasObstacleIntersection) {
                  ...
                  // Orange school
                  nearestSchool.feature.setStyle(
                    new Style({
                      image: new CircleStyle({
                        radius: 8,
                        fill: new Fill({ color: "#FF8C00" }),
                ...
                // Add zoom-dependent distance label style
                ...
                // Set style that considers zoom level
                labelFeature.setStyle(() => {
                  const zoom = mapRef.current?.getView().getZoom() || 0;
                  return zoom >= DISTANCE_LABEL_MIN_ZOOM
                    ? labelStyle
                    : undefined;
                });
                ...
                connectionsCreated++;
                const status = nearestSchool.hasObstacleIntersection
                  ? "🚧 HAS OBSTACLES"
                  : "✅ CLEAR PATH";
                console.log(
                  `✅ ${anganwadiName} → ${
                    nearestSchool.name
                  } (${nearestSchool.distance.toFixed(1)}km) ${status}`
                );
              } catch (error) {
    ...
    const generateBufferReport = () => {
      ...
      console.log(
        "📊 Generating comprehensive anganwadi buffer analysis report..."
      );
      ...
      const reportData = anganwadis.map((aw, index) => {
        ...
        schools.forEach((school) => {
          ...
          if (distance <= ANALYSIS_RADIUS) {
            ...
            schoolsInRange.push({
              name:
                schoolProps.school_nam || `School ${schoolsInRange.length + 1}`,
              distance: distance,
        ...
        return {
          ...
          village:
            awProps.vill_nam_e || awProps.village_na || "Unknown Village",
          ...
          nearestSchoolDistance: nearestSchool
            ? nearestSchool.distance.toFixed(2)
            : "N/A",
          ...
          schoolsWithinRadius: schoolsInRange
            .map((s) => `${s.name} (${s.distance.toFixed(1)}km, ${s.category})`)
            .join(" | "),
          ...
          primarySchools: schoolsInRange.filter((s) => s.category === "Primary")
            .length,
          upperPrimarySchools: schoolsInRange.filter(
            (s) => s.category === "Upper Primary"
          ).length,
          secondarySchools: schoolsInRange.filter(
            (s) => s.category === "Secondary"
          ).length,

          // Management Types
          governmentSchools: schoolsInRange.filter(
            (s) =>
              s.management &&
              typeof s.management === "string" &&
              s.management.toLowerCase().includes("government")
          ).length,
          privateSchools: schoolsInRange.filter(
            (s) =>
              s.management &&
              typeof s.management === "string" &&
              s.management.toLowerCase().includes("private")
          ).length,

          // Service Recommendations
          recommendedAction:
            nearestDistance > 5
              ? "Urgent: Establish new school or improve transport"
              : nearestDistance > 3.5
              ? "Monitor: Consider additional facilities"
    ...
    const exportBufferReport = () => {
      ...
      if (!data.length) {
        alert(
          "No data available for export. Please load anganwadi and school data first."
        );
        return;
      }

      console.log(
        "📤 Exporting comprehensive anganwadi buffer analysis report..."
      );
      ...
      const reportDate = `Assessment Date: ${new Date().toLocaleDateString(
        "en-IN",
        {
          year: "numeric",
          month: "long",
      ...
      const optimalCoverageCount = data.filter(
        (d) =>
          d.accessibilityStatus === "Excellent" ||
          d.accessibilityStatus === "Good"
      ).length;
      const interventionRequiredCount = data.filter(
        (d) =>
          d.serviceLevel === "Service Gap" || d.serviceLevel === "Critical Gap"
      ).length;
      const criticalGapCount = data.filter(
        (d) => d.hasSchoolAccess === "No"
      ).length;
      const avgInstitutionsPerAWC = (
        data.reduce((sum, d) => sum + d.totalSchoolsIn5km, 0) / data.length
      ).toFixed(2);

      const executiveSummary = [
        `AWC Units with Optimal Educational Access: ${optimalCoverageCount} units (${(
          (optimalCoverageCount / data.length) *
          100
        ).toFixed(1)}%)`,
        `AWC Units Requiring Policy Intervention: ${interventionRequiredCount} units (${(
          (interventionRequiredCount / data.length) *
          100
        ).toFixed(1)}%)`,
        `AWC Units with Critical Service Gaps: ${criticalGapCount} units (${(
          (criticalGapCount / data.length) *
          100
        ).toFixed(1)}%)`,
        `Average Educational Institutions per AWC: ${avgInstitutionsPerAWC} institutions`,
        `Geographic Coverage Efficiency: ${(
          100 -
          (criticalGapCount / data.length) * 100
        ).toFixed(1)}%`,
        `Infrastructure Development Priority Score: ${
          interventionRequiredCount > data.length * 0.3
            ? "High"
            : interventionRequiredCount > data.length * 0.15
            ? "Medium"
            : "Low"
        }`,
      ];
      ...
      alert(
        `🏛️ GOVERNMENT ASSESSMENT REPORT GENERATED\n\n� EXECUTIVE SUMMARY:\n• Total AWC Units Assessed: ${
          data.length
    ...
    const checkSchoolInfrastructureIntersections = () => {
      try {
        ...
        const schoolFeatures = (
          schoolLayer.getSource() as VectorSource
        ).getFeatures();
        const railFeatures = (
          railLayer.getSource() as VectorSource
        ).getFeatures();
        const riverFeatures = (
          riverLayer.getSource() as VectorSource
        ).getFeatures();
        const roadFeatures = (
          roadLayer.getSource() as VectorSource
        ).getFeatures();
