...
export const PreschoolMapView = forwardRef<any, PreschoolMapViewProps>(({
  ...
  const createEducationalClusterLayer = (points: ClusterPoint[], layerName: string, color: string) => {
    if (!mapRef.current) return null;
    
    // Remove existing cluster layer if it exists
    const existingLayer = refs.current[`${layerName}ClusterLayer` as keyof typeof refs.current] as VectorLayer | null;
    if (existingLayer) {
      mapRef.current.removeLayer(existingLayer);
    }
    ...
    // Store reference
    refs.current[`${layerName}ClusterLayer` as keyof typeof refs.current] = clusterLayer as any;
  ...
  const upsertLayer = (key: string, make: () => VectorLayer) => {
    ...
    if (!map) return null;
    let layer = refs.current[key as keyof typeof refs.current] as VectorLayer | null;
    if (!layer) {
      layer = make();
      ...
      const onReady = () => {
        ...
        if (feats?.length) {
          ...
          if (selectedDistrict) {
            ...
            mapRef.current
              .getView()
              .fit(src.getExtent(), { padding: [50, 50, 50, 50], duration: 700 });
          }
        }
  ...
  useEffect(() => {
    ...
    if (layer) {
      ...
      fetchJSON(wfsUrl({ typeName: "ch_dep_data:cg_village_boundary", cql }), {
        ...
        .catch((e) => (e.name === "AbortError" ? undefined : console.error(e)));
    }
    return () => {
  ...
  useEffect(() => {
    if (!selectedVillage || !refs.current.villageSource || !mapRef.current) return;
  ...
  useEffect(() => {
    ...
    fetchAnganwadiWithFallback({
      ...
      .then((json) => {
        ...
        const points: ClusterPoint[] = feats.map((feature, index) => {
  ...
  }, [layersVisibility.schools, selectedDistrict, selectedVillage, defaultSchoolStyle]);
  ...
  const connectAnganwadiToNearestSchool = () => {
    ...
    const conn = upsertLayer(
      "connectionLayer",
      () =>
        new VectorLayer({ source: new VectorSource(), style: styleDashLine() })
    );
    if (conn) {
  ...
  const generateBufferReport = () => {
    ...
    const data = aSrc.getFeatures().map((aw) => {
      ...
      const inside = schools.filter((s) => turf.booleanPointInPolygon(s, buf));
  ...
  const exportBufferReport = () => {
    ...
    const rows = [
      ["Anganwadi Name", "District", "Village", "Number of Schools", "Schools"],
      ...data.map((r) => [
        r.anganwadiName,
        r.district,
        r.village,
  ...
  const clearMap = () => {
    ...
    if (mapRef.current) {
      mapRef.current
        .getView()
        .animate({ center: DEFAULT_CENTER, zoom: DEFAULT_ZOOM, duration: 600 });
    }
  };
